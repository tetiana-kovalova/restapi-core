...
class TestBooking(BaseTest):
    ...
    def test_5_delete_booking(self):
        booking_id = random.choice([obj.bookingid for obj in Booking().get_all_bookings()[0]])
        content, status_code = Booking().delete_booking(booking_id)

        assert_equals(status_code, 201)
        booking_ids = [obj.bookingid for obj in Booking().get_all_bookings()[0]]
        assert_not_in(booking_id, booking_ids)
